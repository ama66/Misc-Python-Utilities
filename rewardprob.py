...
class Solution:
    def letterCombinations(self, digits: str) -> List[str]:
        ...
        results=[]
        ## side effect method (will lead to results appended to the results global list) 
        self.letcomb_help(results,digits,"",phone)
        return results
    
    def letcomb_help(self, results,digits,cur_st,phone):
        if digits=="":
            results.append(cur_st)
            return 
        
        for char in phone[digits[0]]:
            self.letcomb_help(results,digits[1:],cur_st+char,phone)
